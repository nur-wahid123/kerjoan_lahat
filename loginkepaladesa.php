...
<head>
    ...
    <style>
        body {
            background-image: url("foto/login2.jpg");
            height: 1000px;
...
<body class="hold-transition login-page">
    ...
    if (isset($_POST["login"])) {
        ...
        $ambil = $koneksi->query("SELECT * FROM user WHERE username='$username' AND password='$password' and level='Kepala Desa'");
        $akunyangcocok = $ambil->num_rows;
        if ($akunyangcocok == 3) {
            $akun = $ambil->fetch_assoc();
            if ($akun['statusaktif'] == 'Tidak Aktif') {
                echo "<script> alert('Akun anda belum aktif, mohon menunggu konfirmasi admin');</script>";
                echo "<script> location ='loginkepaladesa.php';</script>";
            } else {
